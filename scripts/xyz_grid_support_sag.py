...
def initialize(script):
    ...
    for scriptDataTuple in scripts.scripts_data:
        if (
            os.path.basename(scriptDataTuple.path) == "xy_grid.py"
            or os.path.basename(scriptDataTuple.path) == "xyz_grid.py"
        ):
            xy_grid = scriptDataTuple.module
            sag_guidance_scale = xy_grid.AxisOption(
                "SAG Guidance Scale",
                float,
            ...
            sag_mask_threshold = xy_grid.AxisOption(
                "SAG Mask Threshold",
                float,
